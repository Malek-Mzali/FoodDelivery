...
import { selectCartItems, selectTotalPrice, selectCartFees } from "../redux/slices/basketSlice";
import { useNavigation } from "@react-navigation/core";

const CheckoutModal = ({ setModalVisible }) => {
  const totalPrice = useSelector(selectTotalPrice);
  const fees = useSelector(selectCartFees);
    ...
    />
      <View style={tailwind`pb-5  w-full px-4 bg-white pt-4`}>
        ...
        <View style={tailwind`mb-5`}>
          ...
          {(fees !== 0.00 ? <OrderItem name="Delivery fees" value={`$${fees}`}  /> : <></> )}
